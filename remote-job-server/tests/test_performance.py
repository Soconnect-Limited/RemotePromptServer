...
class PerformanceTests(unittest.TestCase):
    ...
    def test_three_jobs_quickly(self) -> None:
        for idx in range(3):
            self.manager.create_job("claude", f"load-{idx}", "perf-device", room_id="room-perf", workspace_path="/tmp")
        self.assertEqual(self.stub.count, 3)

    def test_long_running_prompt_placeholder(self) -> None:
        self.manager.create_job("codex", "long-run", "perf-device-2", room_id="room-perf2", workspace_path="/tmp")
    ...
    def test_timeout_result_sets_failed_status(self) -> None:
        class TimeoutStub(FastStubSessionManager):
            def execute_job(self, runner, prompt, device_id, room_id, workspace_path=None, continue_session=True, settings=None):  # type: ignore[override]
                raise TimeoutError("simulated timeout")

        stub = TimeoutStub()
        manager = JobManager(session_manager=stub)
        manager.create_job("claude", "timeout", "device-timeout", room_id="room-timeout", workspace_path="/tmp")
