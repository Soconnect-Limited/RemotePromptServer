...
class StubSessionManager:
    ...
    def execute_job(self, runner: str, prompt: str, device_id: str, continue_session: bool = True):
        ...
        self.calls.append((runner, device_id, prompt, session_id))
        return {
            "success": True,
            "output": f"ok:{prompt}",
            "error": "",
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_claude_session_resume(self) -> None:
        self.manager.create_job("claude", "first", "dev-a")
        first = self.stub.sessions[("claude", "dev-a")]
        self.manager.create_job("claude", "second", "dev-a")
        second = self.stub.sessions[("claude", "dev-a")]
        self.assertEqual(first, second)

    def test_codex_session_resume(self) -> None:
        self.manager.create_job("codex", "hello", "dev-b")
        self.manager.create_job("codex", "follow", "dev-b")
