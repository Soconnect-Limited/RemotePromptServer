...
5. **柔軟なネットワーク切り替え**: LAN (192.168.x.x) / Tailscale VPN (100.x.x.x) 間のシームレスな切り替え
...
- **フィンガープリント一致時はホスト名検証をスキップ**（LAN/VPN切り替え対応）
...
│     ※ フィンガープリント一致時はホスト名検証スキップ              │
│     → LAN/Tailscale VPN どちらからでも接続可能                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
### ホスト名検証スキップ（LAN/VPN切り替え対応）

証明書のフィンガープリントが一致していれば、ホスト名検証をスキップすることで、
同一証明書で複数のIPアドレス（LAN/Tailscale VPN）からの接続を許可する。

#### 実装箇所

以下の3つのURLSessionDelegateで `SecPolicyCreateBasicX509()` を使用：

1. **CertificatePinningDelegate** (APIClient用)
   ```swift
   // フィンガープリントが一致した場合
...
2. **TestConnectionDelegate** (接続テスト用)
   ```swift
   // 接続テスト時はIPアドレスでもドメイン名でも許可
   let policy = SecPolicyCreateBasicX509()
   SecTrustSetPolicies(serverTrust, policy)
   ```

3. **SimpleTrustDelegate** (BadgeManager用)
   ```swift
   // バッジ更新時のAPI呼び出し用
   let policy = SecPolicyCreateBasicX509()
   SecTrustSetPolicies(serverTrust, policy)
...
- `SecPolicyCreateSSL()` はホスト名検証を行う（SSL/TLSポリシー）
- `SecPolicyCreateBasicX509()` はX.509証明書の構造のみを検証（ホスト名検証なし）
- フィンガープリント検証で証明書の同一性は保証されるため、ホスト名検証は不要
...
#### 1. ATS (App Transport Security) 設定
...
- `NSAllowsLocalNetworking` はBonjour検出やローカル機器通信のための**正当な設定**
- `NSAllowsArbitraryLoads` は使用していない（審査で問題になりやすい）
- ドメイン例外も追加していない
...
|------|----------|----------|
| 証明書の無条件受け入れ | ❌ 行っていない | ✅ |
| フィンガープリント検証 | ✅ Certificate Pinning実装 | ✅ |
| ユーザー明示的信頼 | ✅ 確認ダイアログで信頼 | ✅ |
| 先例アプリ | Termius, Prompt 3等 | ✅ |
...
- [x] `Views/Settings/ServerSettingsView.swift` 新規作成
  ...
  - [x] 証明書情報表示（信頼済みの場合）
    ...
    - [x] 有効期限
  - [x] **証明書信頼リセットボタン**
    - [x] 中央表示のアラートダイアログ（confirmationDialogではなくalert）
    - [x] 確認メッセージ: 「次回接続時に証明書の再確認が必要になります。」
  - [x] **すべての設定をリセットボタン**
    - [x] 中央表示のアラートダイアログ
    - [x] 確認メッセージ: 「サーバー接続情報がすべて削除されます。」
...
- [x] `ViewModels/ServerSettingsViewModel.swift` 新規作成
  ...
  - [x] **TestConnectionDelegate** - 接続テスト用のURLSessionDelegate
    - [x] ホスト名検証スキップ（`SecPolicyCreateBasicX509()`）
    - [x] 証明書フィンガープリント抽出
    - [x] 証明書DERデータ抽出
...
- [x] `Services/CertificatePinningDelegate.swift` 新規作成
  ...
  - [x] **ホスト名検証スキップの実装**
    ...
    completionHandler(.useCredential, credential)
    ```
  - [x] 証明書ミスマッチ通知（`NotificationCenter`経由）
    - [x] `certificateMismatchNotification`
    - [x] `certificateErrorNotification`
...
- [x] 証明書エラー専用ダイアログ（`RoomsListView.swift`内に実装）
  - [x] 表示トリガー: ピンニング済み証明書と異なる証明書を受信
  - [x] NotificationCenter経由で即時通知
  - [x] 表示内容:
    - [x] 「サーバー証明書が変更されました」
    - [x] 保存済みフィンガープリント表示（短縮形）
    - [x] 受信フィンガープリント表示（短縮形）
    - [x] 「これが予期された変更であれば、新しい証明書を信頼してください。」
  - [x] ボタン:
    - [x] 「新しい証明書を信頼する」
    - [x] 「設定を開く」
      - [x] エラーメッセージをクリア（連続アラート防止）
      - [x] 0.5秒遅延後に設定画面を開く（アラート閉じ→シート表示の競合回避）
    - [x] 「キャンセル」
...
- [x] `Services/BadgeManager.swift` 修正
  - [x] **SimpleTrustDelegate** - 独自の軽量URLSessionDelegate
    - [x] ホスト名検証スキップ（`SecPolicyCreateBasicX509()`）
    - [x] APIClientのセッションと競合しない独自セッション使用
  - [x] `Security` フレームワークのインポート追加
...
- [x] `Support/Constants.swift` 修正
  ...
  - [x] `allURLs` プロパティ追加（メイン + 代替URLリスト）
  - [x] `autoFallbackEnabled` プロパティ追加
...
- [x] `Views/RoomsListView.swift` 修正
  ...
  - [x] **証明書エラー時のアラート表示**
    - [x] 「設定を開く」ボタンでエラーメッセージをクリア（連続アラート防止）
    - [x] 0.5秒遅延後にServerSettingsViewを開く
...
- [x] Xcodeプロジェクト設定に追加
  - `NSLocalNetworkUsageDescription = "ローカルネットワーク上のRemotePromptサーバーに接続します。"`

### 5.2 Bonjourサービス宣言（自動検出使用時）

- [x] Xcodeプロジェクト設定に追加
  - `NSBonjourServices = ["_remoteprompt._tcp"]`
...
- [x] **ATS例外は最小限**（ピンニングで対応）
  - [x] `NSAllowsLocalNetworking = true`（Bonjour検出用）
  - [x] `NSExceptionDomains` によるIP例外は**追加しない**（審査リスク増大）
  - [x] `NSAllowsArbitraryLoads` は**使用しない**

#### 5.3.3 App Store審査向け説明テンプレート

- [x] 審査向け説明文を準備（App Review Notesに記載）
  ```
  【日本語】
  このアプリはユーザーが自己ホストするサーバーに接続します。
  サーバーは自己署名証明書を使用しますが、SSH接続と同様に
...
- [x] マルチURL・フォールバック動作
  ...
  - [x] Tailscale接続時 → Tailscale IPで接続確認
  - [x] 自動フォールバック動作確認
    - [x] メインURL到達不可 → 代替URLへ自動切替
    - [x] 切替成功のログ/表示確認
- [x] **LAN/VPN切り替え（ホスト名検証スキップ）**
  - [x] 同一証明書でLAN IPから接続
  - [x] 同一証明書でTailscale IPから接続
  - [x] 証明書リセット不要でシームレス切り替え
...
- [x] `SSL_MODE=commercial` テスト
  - [x] 商用証明書（Let's Encrypt）での起動確認
  - [x] クライアント側でピンニング動作確認
...
| LAN/VPN切り替え時のセキュリティ | フィンガープリント一致が必須、ホスト名検証スキップは二次的 |
...
| フレームワーク | 用途 |
|---------------|------|
| Security.framework | Keychain, 証明書操作, SecPolicyCreateBasicX509 |
