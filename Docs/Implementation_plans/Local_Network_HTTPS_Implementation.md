...
- [ ] `remote-job-server/cert_generator.py` 新規作成
  - [ ] 関数: `generate_self_signed_cert(common_name, san_ips, valid_days=3650)`
    - [ ] RSA 4096bit 秘密鍵生成
    - [ ] X.509 証明書生成（CN=IPアドレス or ホスト名）
    - [ ] SAN (Subject Alternative Name) に**複数IPアドレス対応**
      - [ ] ローカルIP（例: 192.168.11.110）
      - [ ] Tailscale IP（例: 100.100.30.35）※設定可能
      - [ ] ホスト名（オプション）
    - [ ] 有効期限: 10年（3650日）
  - [ ] 関数: `get_certificate_fingerprint(cert_path) -> str`
    - [ ] SHA256フィンガープリントを計算
    - [ ] コロン区切り形式で返却 (例: `A1:B2:C3:...`)
  - [ ] 関数: `ensure_certificate_exists(cert_dir, hostname, san_ips)`
    - [ ] 証明書が存在しなければ自動生成
    - [ ] 既存なら読み込み
  - [ ] 関数: `regenerate_certificate(cert_dir, hostname, san_ips)` ← **証明書ローテーション用**
    - [ ] 既存証明書をバックアップ（`server.crt.bak.YYYYMMDD`）
    - [ ] 新規証明書を生成
    - [ ] 新旧フィンガープリントをログ出力
  - [ ] 関数: `revoke_certificate(cert_dir)` ← **強制失効用**
    - [ ] 証明書ファイル削除
    - [ ] 失効ログ記録
...
- [ ] 必要なシステム要件をドキュメント化
  - [ ] Python 3.9以上
  - [ ] cryptographyライブラリ（Rustビルド環境不要のwheel推奨）
  - [ ] OpenSSL 1.1以上（システム標準で可）
- [ ] ファイルパーミッション設定
  - [ ] 秘密鍵: `chmod 600 server.key`
  - [ ] 証明書: `chmod 644 server.crt`
  - [ ] certsディレクトリ: `chmod 700 certs/`
...
- [ ] `remote-job-server/main.py` 修正
  - [ ] `lifespan()` 内で `ensure_certificate_exists()` 呼び出し
  - [ ] 証明書パスを設定から読み込み
  - [ ] 起動ログにフィンガープリント出力

### 1.4 証明書情報取得API

- [ ] `GET /server/certificate` エンドポイント追加
  - [ ] レスポンス:
    ```json
    {
  ...
  - [ ] 認証不要（初回接続時に呼び出すため）
  - [ ] セキュリティ対策:
    - [ ] レート制限: 10回/分/IP（DoS防止）
    - [ ] 監査ログ: アクセス元IP・タイムスタンプを記録

### 1.4.1 証明書ローテーションAPI

- [ ] `POST /server/certificate/regenerate` エンドポイント追加（管理者用）
  - [ ] 認証: API Key必須
  - [ ] セキュリティ制約:
    - [ ] IP制限: Tailscale/ローカルネットワークからのみ許可（オプション）
    - [ ] レート制限: 1回/時間（証明書再生成の乱用防止）
    - [ ] 監査ログ: 実行者IP、タイムスタンプ、理由を記録
      ```
  ...
  - [ ] 処理フロー:
    - [ ] 旧証明書をバックアップ
    - [ ] 新証明書を生成
    - [ ] クライアントへの通知イベント発火（SSE経由）
    - [ ] サーバー再起動が必要な旨を返却

### 1.4.1.1 SSE証明書変更通知仕様

- [ ] SSE接続要件
  - [ ] エンドポイント: `/events`（既存SSEエンドポイントを流用）
  - [ ] 認証方式: API Key（`x-api-key` ヘッダー）
  - [ ] 再接続バックオフ: 初回1秒、最大30秒まで指数増加
  - [ ] ハートビート: 30秒間隔で `:ping` を送信
- [ ] SSEイベント定義
  - [ ] イベント名: `certificate_changed`
  - [ ] Payload:
    ```json
    {
    ...
    ```
  - [ ] 送信タイミング: `POST /server/certificate/regenerate` 成功直後
  - [ ] 送信対象: 全アクティブSSE接続（ブロードキャスト）
  - [ ] 送信頻度制限: 同一イベントは5分間に1回まで（連続再生成防止）
- [ ] クライアント側再接続ポリシー
...
- [ ] 証明書モード切替設定
  - [ ] `.env` に `SSL_MODE` 追加
    ...
    ```
  - [ ] `auto`: 商用証明書パスが存在すれば使用、なければ自己署名
  - [ ] 商用証明書パス: `certs/config/live/*/fullchain.pem`（既存Let's Encrypt）
  - [ ] 自己署名証明書パス: `certs/self_signed/server.crt`
- [ ] 優先順位:
  1. 環境変数で明示指定されたモード
...
- [ ] `config.py` に `get_ssl_paths()` 関数追加
  ```python
...
- [ ] フォールバック時の安全対策（以下のいずれかを実装）
  - [ ] **オプションA: 明示オプトイン方式（推奨）**
    - [ ] `.env` に `SSL_AUTO_FALLBACK_ENABLED=false`（デフォルト無効）
    - [ ] 商用証明書消失時、フォールバック無効ならサーバー起動失敗
    - [ ] 起動失敗ログ: `[ERROR] Commercial certificate not found. Set SSL_AUTO_FALLBACK_ENABLED=true or SSL_MODE=self_signed to use self-signed certificate.`
  - [ ] **オプションB: 警告付き自動フォールバック**
    - [ ] フォールバック発生時にサーバー起動は成功
    - [ ] ただし、起動時に `[WARN] SECURITY: Falling back to self-signed certificate. Existing clients may need to re-verify.` を出力
    - [ ] `/health` エンドポイントに `certificate_fallback_warning: true` を追加
...
- [ ] SSEイベント `certificate_mode_changed` Payload定義
  ```json
  {
  ...
  ```
  - [ ] `mode_before`: 変更前のモード（commercial/self_signed）
  - [ ] `mode_after`: 変更後のモード
  - [ ] `reason`: 変更理由（`fallback_commercial_not_found`, `manual_switch`, `config_change`）
  - [ ] `triggered_at`: イベント発生日時（ISO 8601）

### 1.4.3 `/health` エンドポイント拡張

- [ ] `GET /health` レスポンスに証明書関連フィールド追加
  - [ ] 既存フィールドを維持しつつ以下を追加:
    ```json
    {
      "status": "ok",
    ...
    ```
  - [ ] `ssl_mode`: 現在使用中のSSLモード
  - [ ] `certificate_fallback_warning`: autoモードでフォールバック発生時 `true`
  - [ ] `certificate_fingerprint`: 現在の証明書フィンガープリント
...
- [ ] `config.py` に設定追加
  - [ ] `ssl_cert_path: str`
  - [ ] `ssl_key_path: str`
  - [ ] `ssl_auto_generate: bool`
  - [ ] `server_hostname: str`
  - [ ] `server_san_ips: List[str]` - カンマ区切りで複数IP指定可能
...
- [ ] `Models/ServerConfiguration.swift` 新規作成
  - [ ] `struct ServerConfiguration: Codable, Identifiable`
    - [ ] `id: UUID`
    - [ ] `name: String`
    - [ ] `url: String` - メインURL（ローカル or Tailscale）
    - [ ] `alternativeURLs: [String]` - 代替URL（フォールバック用）
    - [ ] `apiKey: String`
    - [ ] `certificateFingerprint: String?`
    - [ ] `isTrusted: Bool`
    - [ ] `autoFallback: Bool` - 接続失敗時に代替URLを自動試行
    - [ ] `lastConnected: Date?`
    - [ ] `createdAt: Date`

### 2.2 サーバー設定永続化

- [ ] `Services/ServerConfigurationStore.swift` 新規作成
  - [ ] Keychain保存（APIキー、証明書データ）
  - [ ] UserDefaults保存（URL、名前等の非機密情報）
  - [ ] 関数: `save(_ config: ServerConfiguration)`
  - [ ] 関数: `load() -> ServerConfiguration?`
  - [ ] 関数: `delete()`
  - [ ] 関数: `saveCertificate(_ data: Data, fingerprint: String)`
  - [ ] 関数: `loadCertificate() -> (Data, String)?`

### 2.3 サーバー設定画面UI

- [ ] `Views/Settings/ServerSettingsView.swift` 新規作成
  - [ ] サーバーURL入力フィールド
    - [ ] プレースホルダー: `https://192.168.11.110:8443`
    - [ ] URLバリデーション（https://必須）
  - [ ] 代替URL入力セクション
    - [ ] 「+ 代替URLを追加」ボタン
    - [ ] 複数URL入力対応（Tailscale等）
    - [ ] 削除ボタン（スワイプ or ボタン）
  - [ ] 自動フォールバック設定
    - [ ] トグルスイッチ
    - [ ] 説明: 「メインURL接続失敗時に代替URLを試行」
  - [ ] API Key入力フィールド
    - [ ] SecureField使用
    - [ ] ペースト対応
  - [ ] 接続テストボタン
...
- [ ] `ViewModels/ServerSettingsViewModel.swift` 新規作成
  - [ ] `@Published var serverURL: String`
  - [ ] `@Published var alternativeURLs: [String]`
  - [ ] `@Published var autoFallback: Bool`
  - [ ] `@Published var apiKey: String`
  - [ ] `@Published var connectionStatus: ConnectionStatus`
  - [ ] `@Published var connectedURL: String?` - 実際に接続できたURL
  - [ ] `@Published var certificateInfo: CertificateInfo?`
  - [ ] `@Published var showCertificateAlert: Bool`
  - [ ] 関数: `testConnection() async`
    - [ ] メインURL試行
    - [ ] 失敗時かつautoFallback有効 → 代替URL順次試行
    - [ ] 接続成功したURLを記録
  - [ ] 関数: `addAlternativeURL(_ url: String)`
  - [ ] 関数: `removeAlternativeURL(at index: Int)`
  - [ ] 関数: `trustCertificate()`
  - [ ] 関数: `saveConfiguration()`

### 2.5 Bonjour自動検出（オプション）

- [ ] `Services/BonjourDiscovery.swift` 新規作成
  - [ ] `NetServiceBrowser` でローカルサーバー検出
  - [ ] サービスタイプ: `_remoteprompt._tcp`
  - [ ] 検出されたサーバーをリスト表示
  - [ ] タップで自動入力

- [ ] サーバー側: Bonjour公開
  - [ ] `remote-job-server/bonjour_publisher.py` 新規作成
  - [ ] zeroconf ライブラリ使用
...
- [ ] `Services/CertificatePinningDelegate.swift` 新規作成
  - [ ] `class CertificatePinningDelegate: NSObject, URLSessionDelegate`
  - [ ] 関数: `urlSession(_:didReceive:completionHandler:)`
    - [ ] サーバー証明書を取得
    - [ ] 保存済みフィンガープリントと比較
    - [ ] 一致: 接続許可
    - [ ] 不一致: 接続拒否 + エラー通知
...
- [ ] `Utilities/CertificateValidator.swift` 新規作成
  - [ ] 関数: `extractFingerprint(from trust: SecTrust) -> String?`
    - [ ] SecTrustから証明書取得
    - [ ] SHA256ハッシュ計算
    - [ ] コロン区切り文字列に変換
  - [ ] 関数: `extractCertificateData(from trust: SecTrust) -> Data?`
    - [ ] DER形式でエクスポート
  - [ ] 関数: `compareCertificates(stored: Data, received: SecTrust) -> Bool`

### 3.3 証明書確認アラート

- [ ] `Views/Settings/CertificateConfirmationView.swift` 新規作成
  - [ ] モーダル表示
  - [ ] 表示内容:
    - [ ] 警告アイコン
    - [ ] 「サーバーの証明書を検証できません」メッセージ
    - [ ] フィンガープリント（SHA256）
    - [ ] 「サーバー側の表示と一致していますか？」
  - [ ] ボタン:
    - [ ] 「キャンセル」→ 接続中止
...
- [ ] `Views/Settings/CertificateChangedAlertView.swift` 新規作成
  - [ ] 表示トリガー: ピンニング済み証明書と異なる証明書を受信
  - [ ] 表示内容:
    - [ ] **警告アイコン（赤）**
    - [ ] 「サーバーの証明書が変更されました」
    - [ ] 「これは中間者攻撃の可能性があります」
...
- [ ] `ServerConfigurationStore.swift` に追加
  - [ ] 関数: `clearTrustedCertificate()`
    - [ ] Keychainから証明書データ削除
    - [ ] `isTrusted = false` に設定
  - [ ] 関数: `resetAllConfiguration()`
    - [ ] 全設定をクリア（URL、API Key含む）
    - [ ] 初期状態に戻す
- [ ] 設定画面に「証明書信頼をリセット」ボタン追加
  - [ ] 確認ダイアログ: 「次回接続時に再確認が必要になります」
- [ ] 設定画面に「すべての設定をリセット」ボタン追加
  - [ ] 確認ダイアログ: 「サーバー接続情報がすべて削除されます」

### 3.4 APIClient統合

- [ ] `Services/APIClient.swift` 修正
  - [ ] URLSession生成時にカスタムDelegate設定
  - [ ] 証明書エラー時のコールバック追加
  - [ ] 保存済み証明書の読み込み
  - [ ] 設定変更時のセッション再生成
...
- [ ] `RemotePromptApp.swift` 修正
  - [ ] 起動時にサーバー設定をチェック
  - [ ] 未設定 → ServerSettingsView表示
  - [ ] 設定済み → 通常のContentView表示

### 4.2 既存Constants.swift修正

- [ ] `Support/Constants.swift` 修正
  - [ ] ハードコードURL削除
  - [ ] `ServerConfigurationStore`から動的に取得
  ```swift
  static var baseURL: String {
      ServerConfigurationStore.shared.currentConfiguration?.url
...
- [ ] `Support/AppConfiguration.swift` 修正
  - [ ] plist優先 → 保存設定優先に変更
  - [ ] 移行ロジック（既存設定の読み込み）
...
- [ ] 移行元（旧設定）
  - [ ] `RemotePromptConfig.plist` の `RemotePromptBaseURL`
  - [ ] `RemotePromptConfig.plist` の `RemotePromptAPIKey`
  - [ ] Keychain の `device_id`（既存）
- [ ] 移行先（新設定）
  - [ ] `ServerConfigurationStore` の `url`
  - [ ] `ServerConfigurationStore` の `apiKey`（Keychain経由）
  - [ ] `ServerConfigurationStore` の `certificateFingerprint`（新規）
- [ ] 移行フロー
  ```swift
  ...
  ```
- [ ] 移行失敗時のフォールバック
  - [ ] plist読み込み失敗 → 設定画面表示（手動入力促進）
  - [ ] Keychain読み込み失敗 → API Key手動入力促進
  - [ ] エラーログ出力（デバッグ用）
- [ ] 移行完了後の旧設定
  - [ ] plistファイルは残置（アンインストール時に自動削除）
  - [ ] 旧KeychainのAPI Key項目は残置（互換性維持）

### 4.4 設定画面へのナビゲーション

- [ ] `Views/RoomsListView.swift` 修正
  - [ ] 設定ボタン追加（ナビゲーションバー右上）
  - [ ] タップでServerSettingsView表示

### 4.5 接続エラー時の誘導

- [ ] 接続エラー発生時に設定画面へのリンク表示
  - [ ] 「サーバーに接続できません」
  - [ ] 「[設定を確認する]」ボタン
...
- [ ] `Info.plist` に追加
  ...
  <string>ローカルネットワーク上のRemotePromptサーバーに接続します。</string>
  ```
...
- [ ] `Info.plist` に追加
  ...
  <array>
      <string>_remoteprompt._tcp</string>
  </array>
  ```
...
- [ ] **ATS例外は基本的に不要**（ピンニングで対応）
- [ ] ただし、審査時に説明を求められた場合の対応準備
  - [ ] `NSAllowsLocalNetworking = true` は不要（iOS 14以降は別途許可ダイアログで対応）
  - [ ] `NSExceptionDomains` によるIP例外は**追加しない**（審査リスク増大）
...
- [ ] `tests/test_cert_generator.py` 新規作成
  - [ ] 証明書生成テスト
  - [ ] フィンガープリント計算テスト
  - [ ] 証明書読み込みテスト
...
- [ ] `RemotePromptTests/CertificateValidatorTests.swift` 新規作成
  - [ ] フィンガープリント抽出テスト
  - [ ] 証明書比較テスト

- [ ] `RemotePromptTests/ServerConfigurationStoreTests.swift` 新規作成
  - [ ] 保存・読み込みテスト
  - [ ] Keychain操作テスト
