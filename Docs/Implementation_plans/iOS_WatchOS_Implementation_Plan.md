...
- [ ] SSEManager統合
  ...
  class JobDetailViewModel: ObservableObject {
      ...
      private let apiClient = APIClient.shared
      private let sseManager = SSEManager()  // ✅ 追加
      private let jobId: String
  ...
  struct JobDetailView: View {
      ...
      var body: some View {
          ScrollView {
              VStack(alignment: .leading, spacing: 16) {
                  // ✅ SSE接続状態表示
                  if viewModel.sseManager.isConnected {
                      HStack {
                          Circle()
                              .fill(Color.green)
          ...
          .task {
              ...
              DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                  if !viewModel.sseManager.isConnected && viewModel.job?.isRunning == true {
